...
class RetargetingConfig:
    ...
    # Optional DexPilot V2 terms.  Defaults preserve the upstream objective.
    joint_regularization_weight: float = 0.0
    joint_coupling_terms: Optional[List[Dict[str, Any]]] = None
    joint_coupling_weight: float = 0.0
    collision_link_groups: Optional[List[List[str]]] = None
    collision_min_distance: float = 0.0
    collision_weight: float = 0.0
    collision_samples_per_segment: int = 5
    ...
    def build(self) -> SeqRetargeting:
        ...
        if self.type == "position":
            ...
            optimizer = DexPilotOptimizer(
                ...
                joint_regularization_weight=self.joint_regularization_weight,
                joint_coupling_terms=self.joint_coupling_terms,
                joint_coupling_weight=self.joint_coupling_weight,
                collision_link_groups=self.collision_link_groups,
                collision_min_distance=self.collision_min_distance,
                collision_weight=self.collision_weight,
                collision_samples_per_segment=self.collision_samples_per_segment,
            )
        else:
            raise RuntimeError()
